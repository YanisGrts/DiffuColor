...
class double_conv(nn.Module):
    def __init__(self, in_ch, out_ch, t_emb_dim):
        super().__init__()
        self.conv1 = nn.Sequential(nn.Conv2d(in_ch, out_ch, 3, padding=1), nn.BatchNorm2d(out_ch), nn.ReLU(inplace=True))
        self.time_proj = nn.Linear(t_emb_dim, out_ch)  # injects t between the two convs
        self.conv2 = nn.Sequential(nn.Conv2d(out_ch, out_ch, 3, padding=1), nn.BatchNorm2d(out_ch), nn.ReLU(inplace=True))

    def forward(self, x, emb):
        x = self.conv1(x)
        x = x + self.time_proj(emb).unsqueeze(-1).unsqueeze(-1)  # broadcast over H,W
        return self.conv2(x)
