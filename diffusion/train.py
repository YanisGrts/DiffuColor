...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train DDPM colorization model")
    ...
    parser.add_argument("--drop_prob",      type=float, default=0.15,
                        help="Probability of dropping L condition during training (CFG)")
    parser.add_argument("--guidance_scale", type=float, default=2.0,
                        help="CFG guidance scale at sampling time (1.0 = no guidance)")
...
def main():
    ...
    train_dataset = COCOColorizationDataset(
        "../ds/coco/train2017", transform=transform)
    ...
    if args.checkpoint is not None:
        ...
        if isinstance(ckpt, dict) and "model_state" in ckpt:
            ...
            try:
                start_epoch = int(
                    "".join(filter(str.isdigit, basename.split("epoch")[-1].split(".")[0])))
                print(
                    f"[resume] Inferred start epoch {start_epoch} from filename")
            except (ValueError, IndexError):
                print(
                    "[resume] Could not infer epoch from filename; starting epoch counter at 0")

        # Fast-forward scheduler
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=start_epoch + EPOCHS, eta_min=LR / 10)
    ...
    for epoch in range(start_epoch, start_epoch + EPOCHS):
        ...
        print(
            f"Epoch {epoch + 1:3d} | train {avg_train:.4f} | val {avg_val:.4f} | lr {current_lr:.2e}")
        wandb.log({
        ...
        if (epoch + 1) % 5 == 0:
            ckpt_path = os.path.join(
                CHECKPOINT_DIR, f"unet_epoch{epoch + 1}.pt")
            torch.save(
                {
                    "epoch":           epoch + 1,
            ...
            with torch.no_grad():
                ...
                for step in reversed(range(T)):
                    t_batch = torch.full(
                        (8,), step, device=DEVICE, dtype=torch.long)
                    ab_gen = p_sample(
                ...
                for i in range(8):
                    ...
                    images.append(wandb.Image(
                        rgb, caption=f"Image {img_num} - Variation {var_num}"))
